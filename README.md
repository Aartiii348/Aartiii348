<img width="1584" height="396" alt="image" src="https://github.com/user-attachments/assets/db55fd66-c0b9-4dcd-894a-6efea040b819" />

<h1 align="center">👋 Hi, I'm Aarti Somwanshi</h1>
<h3 align="center">Aspiring Data Analyst | Python • SQL • Power BI • Tableau</h3>

---

## ⭐ About Me

I am an **Aspiring Data Analyst** passionate about turning raw data into meaningful insights.  
I enjoy working with **Python, SQL, and visualization tools** to solve real-world problems and create impactful dashboards.

- 🔍 Currently learning **advanced analytics & dashboards**  
- 📊 Love working on **Python data projects**  
- 🚀 Improving every day through hands-on practice  

---

## 🚀 Skills & Tools

### **Programming & Analytics**                        
- 🐍 Python  
- 🗃 SQL  
- 🧮 Pandas / NumPy  
- 📓 Jupyter Notebook  

### **Visualization & BI Tools**
- 📊 Power BI  
- 📉 Tableau  
- 📘 Excel  
- 📈 Data Visualization  

### **Other Skills**
- 🔍 Exploratory Data Analysis (EDA)  
- 🤖 Machine Learning Basics  
---

## 📂 Featured Projects

### 🔹 **Delivery Supply Chain Logistics Dashboard (Tableau)**
Interactive dashboard for delivery insights and logistics optimization.

### 🔹 **Blinkit Sales Analysis (Python)**
Python-based EDA and visualizations for Blinkit sales dataset.

### 🔹 **Swiggy Sales Analysis (Excel)**
A complete Excel dashboard analyzing Swiggy order trends.

### 🔹 **Customer Churn Analysis (Python)**
Identifying at-risk customers using Python + visualization.

---

## 🔗 Connect With Me
- Gmail - somwanshiaarti09@gmail.com
- LinkedIn - https://www.linkedin.com/in/aartisomwanshi-/
- GitHub - https://github.com/Aartiii348
- Tableau Public - https://lnkd.in/gxrFGtrU

## 🏆 Licenses & Certifications
- Data Analytics With Azure and AI (Grow Data Skills)
- Accenture Developer and Technology Job Simulation Certificate 
- Deloitte Data Analytics

*Thank you for visiting my profile! Let's connect and turn data into insights together.*

